...
public class DirectoryListInfosCommand extends AbstractFacadeCommand {
    public DirectoryListInfosCommand() {
        super("directory-list-infos");
    }
    ...
    public void executeImpl(FacadeCommandContext context) throws IOException {
        ...
        if (nPath.isDirectory()) {
            context.setJsonResponse(
                    nPath.list().stream().map(x-> FileInfoCommand.fileInfo(x)).toArray()
            ).sendResponse();
            return;
        }
