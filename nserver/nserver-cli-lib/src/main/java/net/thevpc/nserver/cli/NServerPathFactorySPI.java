...
public class NServerPathFactorySPI implements NPathFactorySPI {
    @Override
    public NCallableSupport<NPathSPI> createPath(String path, ClassLoader classLoader) {
        if (
                path.startsWith("nserver-http:")
                        || path.startsWith("nserver-https:")
        ) {
    ...
    public static class NServerPathSPI implements NPathSPI {
        ...
        final NSHttpClient client;
        final String remotePath;

        private NServerPathSPI(NConnexionString cnx, String remotePath, NSHttpClient client) {
            this.cnx = cnx;
            this.client = client;
        ...
        public NServerPathSPI(NConnexionString cnx) {
            this.cnx = cnx;
            this.client = new NSHttpClient();
            this.client.setUrl(cnx.getHost());
        ...
        public NStream<NPath> list(NPath basePath) {
            return
                    NStream.of(client.listNames(remotePath))
                            .map(x ->
        ...
        public NPathType type(NPath basePath) {
            try {
                NSHttpClient.NFileInfo i = client.getFileInfo(remotePath);
                if (i != null) {
        ...
        public void delete(NPath basePath, boolean recurse) {
            client.exec("nsh", "-c", "rm", "-R", remotePath);
        }

        @Override
        public void mkdir(boolean parents, NPath basePath) {
            client.exec("nsh", "-c", "mkdir", parents ? "-p" : null, remotePath);
        }
